...
- wall-clock per instruction for both VMs at each N
...
### Performance Observations (Phase 1)

Benchmarks for compute-heavy, boundary-light workloads (e.g., tail-recursive countdown) show Thetao VMs outperforming established Yin VMs by ~25%. This is attributed to:
- **Boundary Amortization**: Fusing many reductions into a single `step` via the local `loop/recur` in `reduce-continuation`.
- **Record-based Dispatch**: Using JVM-native field access and `instance?` checks for internal types (`Primitive`, `Closure`, `Node`) instead of map/keyword lookups.
- **Arity-specific Primitives**: Avoiding `apply` overhead for common arities (0-3).

### Future Optimization Targets

- **Adversarial Benchmarks**: Develop tests for high-frequency context switching (ping-pong streams) and deep lexical nesting (variable lookup) to identify the "boundary tax" and $O(N)$ lookup costs.
- **Vector-Native Materialization**: Transition from Record/Map-based continuations to raw Vector-based Tuples matching the stream dimension. This would allow zero-copy persistence and stream-native dispatch, potentially eliminating the boundary tax.
- **Primitive Array Backend**: Investigate packing continuation state (IP, Hash-refs, Local IDs) into primitive `long-arrays` to optimize for cache locality, memory footprint, and hashing performance.
