...
> **Status: Superseded.** This document describes an earlier framing in which `dao.space`
> *is* the query engine — `dao.space/q` / `dao.space/match` as space verbs that fold member
> logs into a `dao.db` value. That framing has been replaced. The current design splits
> three Datomic boundaries: **`dao.space` is the Storage boundary** (a decentralized
> append-only datom log of member streams), and **querying lives in a separate embeddable
> library, `dao.space.query`** (a Peer that reads the space and runs pattern matching +
> Datalog). `dao.db` is eliminated as a concept. See `docs/design/dao.space.md` and
> `docs/design/adr/0001-dao-space-as-storage-boundary.md`.
>
> The material below on resources, typed streams, geometry, and the Linda/JavaSpace
> lessons remains useful. When reading it, substitute: `dao.space/q` / `dao.space/match`
> → `dao.space.query/q` / `match`; "the space answers queries" → "the query library
> answers queries over the space"; and `dao.db` → the in-memory index the query library
> builds.
