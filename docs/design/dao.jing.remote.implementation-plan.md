...
- `#?(:clj (defn call! [client op args] …))` — D1's loop over `call-step`,
  ...
  throws, is the only way out of the function. The exits, in order:
  ...
  `settle!`, throw N6. On `:done`, `completion-value` after `settle!`; on
