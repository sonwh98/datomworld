...
The division of labor is strict *by design*, even though the code hasn't caught up yet (see Status, below): content addressing and storage semantics — `canonical`, `content-hash`, `segment-key`, the segment/root keyspace discipline — are `dao.jing`'s own job (see `dao.jing.md`, *The Segment and Root Keyspace*), true of a single local `KVMem` with no network at all. `dao.jing.dht`'s role is none of that; it is meant to be purely the networking infrastructure — Kademlia routing, peer discovery, UDP transport, hash-verified fetch against untrusted peers — that takes individual, already-content-addressed `dao.jing` stores and makes them jointly reachable as one distributed hash table. One way to say it: `dao.jing.dht` doesn't invent content addressing, it distributes it — today it still implements it too, in the wrong namespace, until the hoist in Status below happens.
...
The protocol itself does not enforce content addressing or immutability; `put!` is documented as "write an entry unconditionally," and the memory backend overwrites and resets `:rev` to 0 on re-`put!`. The DHT backend imposes a stricter contract on its callers, and that restriction must be explicit. The mechanism below (`canonical`, `content-hash`, `segment-key`, `key-class`) is written here today in `src/cljc/dao/jing/dht.cljc` — only `canonical` is actually private (`defn-`); the other three are already public `defn`s, just scoped to this one namespace — but it is not a DHT-specific idea either way: it is `dao.jing`'s own content-addressing discipline, applicable to any backend, and belongs in `dao.jing.cljc` itself (see `dao.jing.md`, *The Segment and Root Keyspace*); the DHT backend would then just be the first caller that actually needs it enforced over an untrusted network:
