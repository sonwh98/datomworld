...
- **Member layout and discovery.** A stream owner acts as a Transactor: it accepts new datoms, indexes them into B-Tree segments, and unconditionally writes those immutable segments to the `IKVStore` (`put!`). It then performs an atomic `cas!` on the stream's mutable root reference to point to the new B-Tree root. **Crucially, two agents never write to the same stream.** If 1,000 agents want to send messages to one agent, they append to 1,000 distinct single-writer streams, and the recipient lazily merges them all on the read side. The `cas!` operation is used solely for the single owner's mechanical safety: to guard the stream's B-Tree against an agent racing itself. Specifically, `cas!` prevents concurrent write corruption if a network retry causes the agent to duplicate a write request, or if a container orchestrator spins up a zombie duplicate of the agent during a split-brain network partition. It is strictly for structural integrity, not agent-to-agent semantic coordination.
