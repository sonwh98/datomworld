...
**Recorded latency ruling (architect mob, 2026-09-20):** this
served-composition first-request latency, up to the configured backoff
ceiling (200 ms in the default composition), is accepted as composition
policy **because this composition wires no serving-path nudge**; a
composition requiring lower latency must lower its ceiling or provide a
serving-path nudge by wrapping the deposit it hands the transport.
