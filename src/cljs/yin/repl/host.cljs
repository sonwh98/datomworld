(ns yin.repl.host
  "The Node composition of the v2 Yin REPL host WebSocket seam.
   ...
   `websocket`; the portable contract is `yin.repl.host.common`.  It presents
  ...
  (:require [dao.stream.ws.node :as node]))
                  ...
                  {:host bind-host
                   ...
                   (fn []
                     (deposit! (if @bound? :listener-error :bind-failed)
                               {:code (if @bound?
                                        :yin.repl.host/listener-error
                                        :yin.repl.host/bind-failed)
                                :message (if @bound?
