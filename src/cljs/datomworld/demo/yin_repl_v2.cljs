(ns datomworld.demo.yin-repl-v2
  "The browser Yin REPL demo, ported to the DaoStream v2 wire (D3/U4 of
   ...
   adapter directly (`dao.stream.v2.ws.browser`) as `:host` rather than
   calling `(yin.repl.v2.host/websocket)`, because that call would hand the
   REPL the wrong adapter here: `yin.repl.v2.host`'s cljs shadow always
   ...
   That said, requiring `yin.repl.v2.driver` below still pulls in
   `yin.repl.v2.host` transitively (`driver.cljc` requires it unconditionally
   ...
   Drives `yin.repl.v2.driver` exactly as `yin.repl.v2.cljc`'s Node host
...
;; The Node host (`yin.repl.v2.cljc`) ticks at `yin.repl.v2/tick-millis`.
;; That namespace is not required here -- it requires `yin.repl.v2.host`,
...
(def browser-adapter
  "The v2 REPL client boundary's `:host`, composed directly from the browser
   WebSocket adapter rather than from `(yin.repl.v2.host/websocket)` -- that
   ...
   which `yin.repl.v2.driver` already pulls in transitively regardless (see
...
(defn location->repl-url
  [{:keys [hostname]} port]
  ;; `yin.repl.v2.connect/parse-url` has no settled `wss://` descriptor form
...
(defn codemirror-editor
  ...
  (let [view-ref (r/atom nil)
        el-ref (atom nil)]
    (r/create-class
      {:display-name "yin-repl-v2-codemirror-editor",
       :component-did-mount
       (fn [_]
  ...
  [:pre
   {:key id,
    :style {:margin 0,
            ...
            :font-size "13px",
            :color (case kind
                     :yin.repl.v2.driver/result "#f8fbff"
                     :yin.repl.v2.driver/response "#f8fbff"
                     :yin.repl.v2.driver/diagnostic "#ff9b9b"
                     "#8b9ab8")}}
   text])
       ...
       [:div {:style {:max-width "760px"}}
        ...
        [:h1
         {:style {:margin "0 0 10px",
                  ...
                  :line-height "1.05"}}
         "CodeMirror speaks Clojure, yin.repl.v2 stays remote"]
        [:p
         {:style {:margin 0,
                  :color "#b7c7e6",
                  :font-size "16px",
         ...
         [:code "yin.repl.v2"] " server. " [:code "(connect \"daostream:ws://...\")"]
