(ns dao.stream.v2.ws.node
  "The Node host adapter for the DaoStream v2 WebSocket transport.
   ...
   between a Node event and a `dao.stream.v2.ws` adapter entry point, and
   exposes:

   * `connect!` -- the `:connect!` seam of `dao.stream.v2.ws/make-attacher`;
...
(defn raw-socket
  "The `{:send! :close!}` view of one host socket, which is the only shape
   `dao.stream.v2.ws` accepts from a host.  `send!` returns the host library's
...
(defn connect!
  "The `:connect!` host seam for `dao.stream.v2.ws/make-attacher` on Node.
