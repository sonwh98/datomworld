  ...
  (let [endpoint (str url "/api/tags")
    ...
    (if error
      ...
      (let [response (json/read-str body :key-fn keyword)]
        (if (= 200 status)
          (:models response)
          (throw (ex-info "Ollama API Error" {:status status :body response})))))))
...
(defn generate
  ...
  [prompt & {:keys [model url timeout stream]
...
(defn chat
  ...
  [messages & {:keys [model url timeout stream]
               :or {model default-model
                    url default-url
                    timeout default-timeout
                    stream false}}]
...
(defn- clean-response [text]
  ...
  (let [cleaned (str/replace text #"(?s)<think>.*?</think>" "")]
    ;; Remove potential markdown code blocks if the model ignores the prompt
    (-> cleaned
        (str/replace #"```clojure" "")
        (str/replace #"```" "")
        (str/trim))))

(defn- remap-entities [datoms]
  (let [now (System/currentTimeMillis)
    ...
    (mapv (fn [[e a v tx m]]
            [(get mapping e e)
             a
             (get mapping v v) ;; Also remap the value if it's a known entity string
             now
             m])
          datoms)))
...
(defn text->datoms
  ...
  (let [response (chat [{:role "system" :content extraction-system-prompt}
    ...
    (try
      (let [raw-datoms (edn/read-string content)]
        (remap-entities raw-datoms))
      (catch Exception e
