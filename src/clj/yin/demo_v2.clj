(ns yin.demo-v2
  "Two Yin VMs (v2, DaoStream v2) cooperatively compute sum(0..100) by passing
   ...
   - The evaluator is `yin.vm.v2.ast-walker`. `yin.vm.v2` is the ast-walker
     slice; `register` and `stack` are not ported.
   - The shared stream is a real `dao.stream.v2` medium, not a bare vector:
