...
(defn start-server!
  [port]
  (let [handlers
        {:op/add +, :op/multiply *, :op/uppercase clojure.string/upper-case}
        endpoint (ds/open!
                   {:dao.stream/type :websocket, :mode :listen, :port port})
    ...
    (future
      (loop [cursor {:position 0}]
        (if @stop-atom
          (do (ds/close! endpoint) (println "Server stopped"))
          (do (Thread/sleep 10)
              (if-let [req-result (dao-apply/next-request endpoint cursor)]
                (if (map? req-result)
                  (let [request (:ok req-result)
                    ...
                    (try (let [response (dao-apply/dispatch-request handlers
                                                                    request)]
                           (println (str "Handled " op
                                         " " args
                                         " → " (dao-apply/response-value
                                                 response)))
                           (dao-apply/put-response! endpoint response))
                         (catch Exception e
                           (println (str "Error handling " op
                                         ": " (ex-message e)))
                           (dao-apply/put-response!
                             endpoint
