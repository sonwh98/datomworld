  ...
  (let [;; Materialize and index datoms by entity
    ...
    (letfn [(compile-node [e]
              (let [node-type (get-attr e :yin/type)]
                (case node-type
                  :literal
                  (emit! [:push (get-attr e :yin/value)])

                  :variable
                  (emit! [:load (get-attr e :yin/name)])
                  ...
                  :if
                  (let [test-ref (get-attr e :yin/test)
                  ...
                  ;; VM primitives
                  :vm/gensym
                  (emit! [:gensym (get-attr e :yin/prefix)])

                  :vm/store-get
                  (emit! [:store-get (get-attr e :yin/key)])

                  :vm/store-put
                  (do (emit! [:push (get-attr e :yin/val)])
                      (emit! [:store-put (get-attr e :yin/key)]))

                  ;; Stream operations
                  :stream/make
                  (emit! [:stream-make (get-attr e :yin/buffer)])
...
(comment
  ...
  ;; Application (+ 1 2)
  (ast-datoms->stack-bytecode (ast->datoms {:type :application
  ...
  (ast-datoms->stack-bytecode (ast->datoms {:type :if
...
(comment
  ...
  (def schema
    {:yin/body       {:db/valueType :db.type/ref}
     ...
     :yin/operands   {:db/valueType :db.type/ref :db/cardinality :db.cardinality/many}
...
;; Example 1: Simple literal
  (let [ast {:type :literal :value 42}
    ...
    (println "Query result:"
             (d/q '[:find ?v :where [?e :yin/type :literal]
                    [?e :yin/value ?v]] db)))
  ...
  (let [ast {:type :lambda
    ...
    (println "Lambda body:"
             (d/q '[:find ?name .
                    :where [?lambda :yin/type :lambda]
                    [?lambda :yin/body ?body]
                    [?body :yin/name ?name]] db)))
  ...
  (let [ast {:type :application
    ...
    (println "Operand values:"
             (d/q '[:find [?v ...]
                    :where [?app :yin/type :application]
                    [?app :yin/operands ?op]
                    [?op :yin/value ?v]] db)))
  ...
  (let [ast {:type :if
    ...
    (println "Branch values:"
             (d/q '[:find ?v
                    :where [?if :yin/type :if]
                    (or [?if :yin/consequent ?node]
                        [?if :yin/alternate ?node])
                    [?node :yin/value ?v]] db))))
...
(def reverse-opcode-table
  (into {} (map (fn [[k v]] [v k]) opcode-table)))
    ...
    (letfn [(compile-node [e]
              (let [node-type (get-attr e :yin/type)]
                (case node-type
                  ...
                  ;; Unknown type
                  (throw (ex-info "Unknown node type in register assembly compilation"
                                  {:type node-type :entity e})))))]
...
(comment
  ...
  ;; Application (+ 1 2)
  (ast-datoms->register-assembly (ast->datoms {:type :application
                                               :operator {:type :variable :name '+}
                                               :operands [{:type :literal :value 1}
                                                          {:type :literal :value 2}]}))
  ;; => [[:loadk 0 1]        ; r0 = 1
  ...
  ;; Lambda (fn [x] x)
  (ast-datoms->register-assembly (ast->datoms {:type :lambda
                                               :params ['x]
                                               :body {:type :variable :name 'x}}))
  ...
  (ast-datoms->register-assembly (ast->datoms {:type :if
                                               :test {:type :literal :value true}
                                               :consequent {:type :literal :value 1}
    ...
    (doseq [[idx instr] (map-indexed vector asm-instructions)]
      ...
      (let [[op & args] instr]
        (case op
          ...
          :move
          (let [[rd rs] args]
            (emit! (opcode-table :move) rd rs))

          :closure
          (let [[rd params addr] args]
            (emit! (opcode-table :closure) rd (intern! params))
            (emit-fixup! addr))

          :call
          (let [[rd rf arg-regs] args]
            (emit! (opcode-table :call) rd rf (count arg-regs))
            (doseq [ar arg-regs] (emit! ar)))

          :return
          (let [[rs] args]
            (emit! (opcode-table :return) rs))

          :branch
          (let [[rt then-addr else-addr] args]
...
(defn rbc-step
  ...
  (let [{:keys [bytecode ip regs env store k]} state
    ...
    (if (nil? instr)
      ;; End of assembly without :return - should not happen with compiled assembly
        ...
        (case op
          ...
          (let [[rd rf arg-regs] args
            ...
            (cond
              ...
              (let [result (apply fn-val fn-args)]
                (if (module/effect? result)
                  ...
                  (case (:effect result)
                    :vm/store-put
                    (-> state
                        (assoc-in [:store (:key result)] (:val result))
                        (rbc-set-reg rd (:val result))
                        (update :ip inc))
                    ;; Other effects...
                    (throw (ex-info "Unhandled effect in rbc-step" {:effect result})))
                  ;; Regular value
                  (-> state
              ...
              (let [{:keys [params body-addr env bytecode]} fn-val
                ...
                (-> state
                    (assoc :regs [])  ; fresh register frame (grows as needed)
                    (assoc :k new-frame)
                    (assoc :env new-env)
                    (assoc :ip body-addr)))

              :else
              (throw (ex-info "Cannot call non-function" {:fn fn-val}))))
          ...
          (let [[rs] args
            ...
            (if (nil? k)
              ...
              (assoc state :halted true :value result)
              ;; Restore caller context
              (let [{:keys [return-reg return-ip saved-regs saved-env parent]} k]
                (-> state
          ...
          ;; Unknown instruction
          (throw (ex-info "Unknown assembly instruction" {:op op :instr instr})))))))
...
(comment
  ;; Register assembly VM exploration

  ;; Simple: load constant
...
;; =============================================================================
;;
;; Bytecode is a flat vector of integers produced by register-assembly->bytecode.
    ...
    (if (nil? op)
      ...
      (case (int op)
        ...
        0 (let [rd (get bc (+ ip 1))
            ...
            (-> state (rbc-set-reg rd v) (assoc :ip (+ ip 3))))
        ...
        3 (let [rd        (get bc (+ ip 1))
                ...
                closure   {:type :rbc-closure
                           ...
                           :bc bc
                           :pool pool}]
            (-> state (rbc-set-reg rd closure) (assoc :ip (+ ip 4))))

        ;; call: rd = fn(args...)
        4 (let [rd   (get bc (+ ip 1))
                rf   (get bc (+ ip 2))
                argc (get bc (+ ip 3))
                fn-args (loop [i 0 args (transient [])]
                          (if (< i argc)
                            (recur (inc i)
                                   (conj! args (rbc-get-reg state (get bc (+ ip 4 i)))))
                            (persistent! args)))
                fn-val (rbc-get-reg state rf)
                next-ip (+ ip 4 argc)]
            (cond
                ...
                (if (module/effect? result)
                  (case (:effect result)
                    ...
                    (-> state
                        ...
                        (rbc-set-reg rd (:val result))
                        (assoc :ip next-ip))
                    (throw (ex-info "Unhandled effect in rbc-step-bc" {:effect result})))
                  (-> state
                      (rbc-set-reg rd result)
                      (assoc :ip next-ip))))
