...
(defn ingest-next-program
  ...
  (let [cursor (or (:in-cursor vm) {:position 0})
    ...
    (cond (map? result) (let [cursor' (:cursor result)
          ...
          (throw (ex-info "VM ingress cursor fell behind the input stream"
                          {:cursor cursor, :vm-model (:vm-model vm)}))
          :else (throw
                  (ex-info
                    "Unexpected DaoStream response while ingesting VM program"
                    {:result result, :vm-model (:vm-model vm)})))))
