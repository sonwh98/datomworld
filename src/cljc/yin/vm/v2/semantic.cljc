(ns yin.vm.v2.semantic
  "Linear CESK interpreter for executable datoms on DaoStream v2
   ...
   lowering (`yin.vm.v2.linearize`, Phase 2) is composition-supplied at the
...
(defn vm-load-program
  "Load one `:yin.code/*` batch: validate it, decode it, store the image under
   ...
   This is the loader a composition hands to `dao.stream.v2.observer/
...
(defn- vm-eval
  "Resume loaded work. An AST is refused: lowering belongs to
   `yin.vm.v2.linearize` (Phase 2) and is composed in at the observer
   boundary, so this evaluator never learns which form travels."
  [vm ast]
  (when ast
    (throw (ex-info
             "The semantic VM executes :yin.code/* segments: lower :yin/* ASTs with yin.vm.v2.linearize (Phase 2) or call vm-load-program with a code batch"
             {:ast ast})))
...
(defn create-vm
  "Create a new SemanticVM.
   ...
   to `dao.stream.v2.observer`), and construction is all-or-nothing — the FFI
