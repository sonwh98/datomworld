(ns yin.vm.v2.runtime-adapter
  "Wrap VM wait-set entries as dao.runtime.v2 tasks.
...
(defn terminal-resume-outcome
  "The outcome a parked retry terminated with, when it is one the immediate
   ...
   `dao.runtime.v2` wakes a task for every outcome except the two that can
...
(defn vm-task
  "Wrap a VM entry as a dao.runtime.v2 task."
  [vm-entry restore-fn]
...
(defn enqueue-woken-vm-entries
  "Translate raw woken entries into dao.runtime.v2 ready-queue entries using
