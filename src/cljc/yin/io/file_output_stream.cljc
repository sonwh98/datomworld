...
(defn file-output-stream
  ...
  ([path opts]
   (merge {:effect :io/file-output-stream, :path path}
          (if (number? opts) {:chunk-size opts} opts))))


(def _init
  (module/register-module! 'yin.io {'file-output-stream file-output-stream}))


#?(:clj (do (defn- handle
              [state effect opts]
              (let [stream-id (:id opts)
                    stream (ds/open! {:dao.stream/type :file-output-stream,
                                      :path (:path effect)})
