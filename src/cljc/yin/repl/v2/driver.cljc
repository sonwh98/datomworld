...
(defn- rpc-client
  [state]
  (get-in state [:adapter :yin.repl.v2.adapter/rpc]))
...
(defn- abandon-unsent
  ...
  [state reason]
  (if (rpc-client state)
    (update-in state [:adapter :yin.repl.v2.adapter/rpc] rpc/abandon-unsent reason)
    state))
  ...
  (if-not (:adapter state)
    ...
    (let [state (cond-> state
      ...
      (publish-adapter-events
        (assoc state :adapter (:yin.repl.v2.adapter/state published))))))
...
(defn- disconnect
  ...
  (cond
    ...
    (-> state
        ...
        (abandon-unsent :yin.repl.v2.driver/operator-disconnect)
...
(defn- connect-failure
  [state result]
  (publish state :yin.repl.v2.driver/notice
...
(defn- reattach
  ...
  (let [result (connect/reattach (:connection state) (rpc-client state))]
    (if-not (= :yin.repl.v2.connect/reattached (get result connect/outcome-key))
      (connect-failure state result)
      (-> state
          (assoc :connection (get result connect/connection-key))
          (assoc-in [:adapter :yin.repl.v2.adapter/rpc] (get result connect/client-key))
          ...
          (abandon-unsent :yin.repl.v2.driver/abandoned-on-reattach)
          (assoc :outstanding nil :retrying nil)
          (publish :yin.repl.v2.driver/notice (str "Reattaching to " url))))))
...
(defn- open-connection
  ...
  (let [result (connect/open {:url url :host (:host state)})
    ...
    (if-not (= :yin.repl.v2.connect/attached (get result connect/outcome-key))
      (connect-failure state result)
      (-> state
          (abandon-unsent-now :yin.repl.v2.driver/abandoned-on-fresh-connection)
          (cond-> (pos? dropped)
            (publish :yin.repl.v2.driver/notice
          ...
          (assoc :outstanding nil :retrying nil)
          (publish :yin.repl.v2.driver/notice (str "Attaching to " url))))))
...
(defn- connect-command
  ...
  (let [url (second (adapter/command-form line))]
    (cond
      (not (string? url))
      (publish state :yin.repl.v2.driver/notice connect-usage)
  ...
  (let [command (adapter/local-command line)]
    (cond
      ...
      (cond-> (evaluate-locally state line)
        (:connection state)
        (publish :yin.repl.v2.driver/result
