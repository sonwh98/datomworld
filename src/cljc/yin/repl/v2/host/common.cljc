(ns yin.repl.v2.host.common
  "The portable half of the host WebSocket seam.

   A `.cljs` or `.cljd` shadow replaces `yin.repl.v2.host` wholesale rather than
   ...
   adapter, and the diagnostic for a host that composed none.  `yin.repl.v2.host`
   keeps only `websocket`, the one thing that genuinely differs per build.")


(def missing-code
  "Qualified to `yin.repl.v2.host`, not to this namespace: the code names the
   seam that is unmet rather than the file the constant sits in, and
   `yin.repl.v2.serve` renders it into text that `v2_serve_test` asserts."
  :yin.repl.v2.host/no-websocket-package)
