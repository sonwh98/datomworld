...
(def connection-key :yin.repl.v2.connect/connection)
(def client-key :yin.repl.v2.connect/client)
(def descriptor-key :yin.repl.v2.connect/descriptor)
(def message-key :yin.repl.v2.connect/message)
(def event-key :yin.repl.v2.connect/event)
(def text-key :yin.repl.v2.connect/text)
      ...
      (let [descriptor (get parsed descriptor-key)]
        (if-not (host-common/adapter? host)
          ...
          (let [{:keys [traffic response-cursor lifecycle-cursor]} (boundary traffic)
            ...
            (cond
              (nil? attach!)
              (failure :yin.repl.v2.connect/invalid-composition
              ...
              (let [result (attach! descriptor)
                ...
                (if-not (= :dao.stream/ok (:dao.stream/outcome result))
                  (failure :yin.repl.v2.connect/attach-failed
                           (attach-outcome-message url result))
                  {outcome-key :yin.repl.v2.connect/attached
...
(defn reattachable?
  ...
  [client]
  (= :dao.stream.v2.apply/detached (:terminal client)))
...
(defn reattach
  ...
  (cond
    ...
    (failure :yin.repl.v2.connect/not-reattachable
    ...
    (let [result ((:attach! connection) (:descriptor connection))
      ...
      (if-not (= :dao.stream/ok (:dao.stream/outcome result))
        ...
        {outcome-key :yin.repl.v2.connect/reattached
...
(defn- transition
  ...
  (if (and (contains? terminal-statuses (:status connection))
           (not= :dao.stream.v2.apply/diagnostic lifecycle))
    ...
    (case lifecycle
      ...
      (if (= :established (:status connection))
        ...
        [(assoc connection :status :established)
         (event :yin.repl.v2.connect/connected (str "Connected to " (:url connection)))])

      :dao.stream.v2.apply/detached
      [(assoc connection :status :detached)
       (event :yin.repl.v2.connect/detached
      ...
      [(assoc connection :status :ended)
       (event :yin.repl.v2.connect/ended
      ...
      [(assoc connection :status :not-found)
       (event :yin.repl.v2.connect/not-found
...
(defn- decode-lifecycle
  ...
  (try
    (let [decoded ((:decode connection) value)]
      (when (= :dao.stream.v2.rpc/lifecycle (get decoded rpc/event-key))
           ...
           (case outcome
             ...
             (recur (dec remaining)
                    (assoc connection
                           ...
                           :ledger outcome)
                    (conj events
                          (event :yin.repl.v2.connect/notice
