...
(defrecord AsyncStream
  [ch]

  DaoStream

  (write! [this val] (async/put! ch val))
  ...
  (read!
    ...
    (let [out (async/promise-chan)]
      (async/take! ch
                   (fn [v]
                     (if (nil? v) (async/close! out) (async/put! out v))))
      out))


  (close! [this] (async/close! ch)))
...
(defn stream-seq
  ...
  ((fn step
     []
     (lazy-seq (let [val-ch (read! stream)
                     v (clojure.core.async/<!! val-ch)]
                 (if (nil? v) nil (cons v (step))))))))
