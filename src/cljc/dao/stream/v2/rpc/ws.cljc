(ns dao.stream.v2.rpc.ws
  "The WebSocket decoder for the DaoStream v2 RPC core.
   ...
   of `dao.stream.v2.rpc` before that core's transition algebra sees them,
  ...
  (:require [dao.stream.v2.rpc :as rpc]))
...
(def lifecycle-translation
  "Exact translation of the deposited vocabulary into the neutral lifecycle
   vocabulary owned by `dao.stream.v2.apply`.
  ...
  {opened-event :dao.stream.v2.apply/established
   accepted-event :dao.stream.v2.apply/established
   closed-event :dao.stream.v2.apply/detached
   ended-event :dao.stream.v2.apply/ended
   error-event :dao.stream.v2.apply/diagnostic
   not-found-event :dao.stream.v2.apply/not-found
   transport-error-event :dao.stream.v2.apply/transport-error})


(def malformed-envelope-code :dao.stream.v2.rpc.ws/malformed-envelope)
(def unhandled-event-code :dao.stream.v2.rpc/unhandled-event)
...
(defn decoder
  "Return the envelope decoder for one attachment identity.
   ...
   an event constructed by `dao.stream.v2.rpc`:
...
(def decode
  "Alias supplying the `:decode` slot of `dao.stream.v2.rpc/client-state`."
  decoder)
...
(defn rebind
  "Reattach a detached WebSocket client to a fresh `attach!` result.
   ...
   boundary deposits."
  [state attach-result]
  (if-not (= :dao.stream.v2.apply/detached (:terminal state))
    state
    (let [attach (checked-attach attach-result)
          me (:dao.stream/attachment attach)]
