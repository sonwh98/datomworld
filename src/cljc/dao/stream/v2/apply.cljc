...
(def id-key :dao.stream.v2.apply/id)
(def op-key :dao.stream.v2.apply/op)
(def args-key :dao.stream.v2.apply/args)
(def ok-key :dao.stream.v2.apply/ok)
(def error-key :dao.stream.v2.apply/error)
(def code-key :dao.stream.v2.apply/code)
(def message-key :dao.stream.v2.apply/message)
...
(defn response?
  "True when value is a valid response envelope.  A response contains exactly
   one of `:dao.stream.v2.apply/ok` or `/error`; nil is a valid success value."
  [value]
  (and (map? value)
       (correlation-id? (get value id-key))
...
(def request-key :dao.stream.v2.apply/request)
(def response-key :dao.stream.v2.apply/response)
...
(defn put-request!
  ...
  (if (request? request-value)
    ...
    {:dao.stream.v2.apply/outcome :dao.stream.v2.apply/invalid-request
     :dao.stream.v2.apply/value request-value}))
...
(defn put-response!
  ...
  (if (response? response-value)
    ...
    {:dao.stream.v2.apply/outcome :dao.stream.v2.apply/invalid-response
     :dao.stream.v2.apply/value response-value}))
...
(defn dispatch-request
  ...
  (cond
    ...
    (let [id (request-id request-value)
      ...
      (if-not (fn? handler)
        (error-response id :dao.stream.v2.apply/unknown-operation
        ...
        (try
          ...
          (catch #?(:cljd Object :clj Throwable :cljs :default) _
            (error-response id :dao.stream.v2.apply/handler-error
                            "Handler failed")))))))
...
(defn- step-result
  ...
  (merge {:dao.stream.v2.apply/outcome outcome
          :dao.stream.v2.apply/state state}
         extra))
...
(defn- deliver-pending
  ...
  (let [response (:pending-response state)
    ...
    (case outcome
      :dao.stream/ok
      (step-result :dao.stream.v2.apply/responded
                   (-> state
                       (assoc :request-cursor (:pending-successor state))
                       clear-pending)
                   :dao.stream.v2.apply/response response
                   :dao.stream.v2.apply/append put-result)

      :dao.stream/full
      (step-result :dao.stream.v2.apply/pending-response state
                   :dao.stream.v2.apply/response response
                   :dao.stream.v2.apply/append put-result)
      ...
      (step-result :dao.stream.v2.apply/response-undeliverable
                   (-> state
                       (assoc :request-cursor (:pending-successor state)
                              :terminal outcome)
                       clear-pending)
...
(defn- terminal-result
  ...
  (step-result :dao.stream.v2.apply/terminal
               (assoc state :terminal outcome)
               :dao.stream.v2.apply/read read-result))
...
(defn serve-once!
  ...
  (cond
    (:terminal state)
    (step-result :dao.stream.v2.apply/terminal state)
      ...
      (case outcome
        ...
        (let [request-value (:dao.stream/value read-result)
          ...
          (if-let [response (dispatch-request handlers request-value)]
            ...
            (step-result :dao.stream.v2.apply/malformed-request
                         (-> state
        ...
        :dao.stream/blocked
        (step-result :dao.stream.v2.apply/idle state
                     :dao.stream.v2.apply/read read-result)

        :dao.stream/gap
        (step-result :dao.stream.v2.apply/gap
                     (-> state
                         (assoc :request-cursor (:dao.stream/cursor read-result))
                         (update :diagnostics conj
                                 {:dao.stream.v2.apply/code
                                  :dao.stream.v2.apply/request-gap}))
                     :dao.stream.v2.apply/read read-result)
