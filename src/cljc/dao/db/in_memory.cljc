          ...
          (let [c (dao-db/compare-vals (datom-v d1) (datom-v d2))]
            (if (zero? c)
              (let [c (cmp-long-field (datom-t d1) (datom-t d2))]
                (if (zero? c) (cmp-long-field (datom-m d1) (datom-m d2)) c))
              c))
          c))
      c)))
      ...
      (let [c (cmp-long-field (datom-e d1) (datom-e d2))]
        (if (zero? c)
          (let [c (cmp-keyword-field (datom-a d1) (datom-a d2))]
            (if (zero? c)
              (let [c (dao-db/compare-vals (datom-v d1) (datom-v d2))]
                (if (zero? c) (cmp-long-field (datom-t d1) (datom-t d2)) c))
              c))
          c))
      c)))
...
(defn- subseq-from
  ...
  (let [s (seq sorted-set)] (drop-while #(neg? (cmp % sentinel)) s)))
...
(defrecord InMemoryDaoDB
  [eavt          ; sorted-set of Datom by [e a v t
   ;; m]
   aevt          ; sorted-set of Datom by [a e v t
   ;; m]
   avet          ; sorted-set of Datom by [a v e t
   ;; m] — indexed + ref attrs
   vaet          ; sorted-set of Datom by [v a e t
   ;; m] — ref attrs only
   meat          ; sorted-set of Datom by [m e a v
   ;; t] — m != 0 only
   log           ; vector of [t added retracted],
   ;; one entry per committed tx
   ;; (temporal index)
   schema        ; {attr-kw {:db/valueType ...
   ;; :db/cardinality ...}} — cache
   next-t        ; integer, monotonic tx counter
   next-eid      ; integer, next permanent entity ID
   ;; (starts at 1025)
   ref-attrs     ; set of keywords with :db.type/ref
   card-many     ; set of keywords with
   ;; :db.cardinality/many
   unique-attrs  ; set of keywords with :db/unique
   indexed-attrs ; set of keywords with :db/index
   ;; true
   ])
...
(defn build-schema-cache
  ...
  (let [ident-datoms (filter #(= :db/ident (:a %)) (seq eavt))
                  ...
                  (let [a (:a d)]
                    (if (contains? schema-attrs a)
                      (let [attr-kw (get eid->kw (:e d))]
                        (if attr-kw (update acc attr-kw assoc a (:v d)) acc))
                      acc)))
                {}
                (seq eavt))
        ref-attrs (into #{}
        ...
        (into #{}
              (keep (fn [[k v]]
                      (when (= :db.cardinality/many (:db/cardinality v)) k))
                    schema))
        unique-attrs (into #{}
                           (keep (fn [[k v]] (when (:db/unique v) k)) schema))
        indexed-attrs (into #{}
                            (keep (fn [[k v]] (when (:db/index v) k)) schema))]
    {:schema schema,
     :ref-attrs ref-attrs,
     :card-many card-many,
...
(defn- avet-attr?
  ...
  [db a]
  (or (contains? (:indexed-attrs db) a) (contains? (:ref-attrs db) a)))
...
(defn- collect-tempids
  ...
  (distinct
    (sort (concat (mapcat (fn [{:keys [e a v m]}]
                            (cond-> []
                              (tempid? e) (conj e)
                              (and (contains? ref-attrs a) (tempid? v)) (conj v)
                              (tempid? m) (conj m)))
                          ops)
                  (mapcat (fn [d]
                            (let [e (:e d)
                              ...
                              (cond-> []
                                (tempid? e) (conj e)
                                (and (contains? ref-attrs (:a d)) (tempid? v))
                                (conj v)
                                (tempid? m) (conj m))))
                          extra-datoms)))))
...
(defn- apply-tempid-map
  ...
  (let [resolve (fn [id] (if (tempid? id) (get tempid-map id id) id))]
    (mapv (fn [op]
            (-> op
                (update :e resolve)
                (update :m resolve)
                (update :v
                        (fn [v]
                          (if (and (contains? ref-attrs (:a op)) (integer? v))
                            (resolve v)
                            v)))))
          ops)))
  ...
  (when (keyword? ident)
    (or (get same-tx-ident->eid ident)
        ...
        (->> (seek-av (:avet db) :db/ident ident)
             (first)
             (:e))
        ;; Fallback to AEVT scan if not indexed yet (e.g. during
        ;; bootstrap/import)
...
(defn run-tx
  ...
  (let [t (:next-t db)
        ...
        ;; Includes idents established in this same tx (e.g. -1 given
        ;; :db/ident :tags in op 1; later op uses :tags as entity —
        ;; resolves to -1, then tempid pipeline promotes it to a permanent
        ;; EID).
        same-tx-ident->eid
        (into {}
              (keep (fn [{:keys [op e a v]}]
                    ...
                    parsed-ops))
        ;; Compute effective-ref-attrs BEFORE Step 2 so keyword idents in
        ;; :v can be resolved for ref attributes defined in the same tx.
        ;; This also respects same-tx :db/valueType retractions.
        effective-ref-attrs
        (effective-ref-attrs-for-tx db parsed-ops same-tx-ident->eid)
        ;; Explicit positive user entity ids must advance allocation
        ;; immediately, otherwise metadata/tempid allocation can reuse them
        ;; in this tx or the next. Scans both entity ID and ref-value IDs
        ;; (respecting same-tx ref schema).
        alloc-base-eid
        (reduce (fn [eid {:keys [op e a v m-raw]}]
                  (let [mx (if (and (= op :db/add) (integer? e) (>= e 1025))
                             (max eid (inc e))
                             eid)
                        mx (if (and (= op :db/add)
        ...
        (reduce (fn [[acc-ops acc-extra eid] op]
                ...
                [[] [] alloc-base-eid]
                ops)
        ;; Steps 4-5: collect and resolve tempids (uses
        ;; effective-ref-attrs)
        ...
        ;; Step 7: apply tempid map to ref-typed extra-datom values. Only
        ;; resolve when the metadata attribute is schema-declared as
        ;; :db.type/ref. Scalar metadata values like {:error/code -1} must
        ;; not be rewritten.
        extra-datoms
        (mapv (fn [d]
                (let [d (update d
                                :m
                                #(if (tempid? %) (get tempid-map % %) %))]
                  (if (contains? effective-ref-attrs (:a d))
                    (let [v (:v d)
                          v' (if (keyword? v)
                               (or (lookup-ident-eid db same-tx-ident->eid v)
                                   (throw (ex-info "Unknown value ident"
                                                   {:ident v})))
                               v)
                          v' (if (tempid? v') (get tempid-map v' v') v')]
                      (assoc d :v v'))
                    d)))
              extra-datoms)
        ...
        op-datoms (mapv (fn [{:keys [op e a v m]}]
                        ...
                        ops)
        ;; Step 8: enforce uniqueness constraints. Compute effective
        ;; unique-attrs for this tx: start from the pre-tx set, add attrs
        ;; gaining :db/unique in this tx, remove attrs losing it. This
        ;; makes same-tx schema changes visible to the uniqueness check.
        effective-unique-attrs
        (if-not (some (fn [{:keys [datom]}] (= :db/unique (:a datom)))
                      op-datoms)
          (:unique-attrs db)
          (let [eid->ident (into {}
                                 (map (fn [d] [(:e d) (:v d)])
        ...
        _ (let [add-claim (fn [m k e]
                            (let [existing (get m k)]
                              (cond (nil? existing) (assoc m k e)
                                    (set? existing) (update m k conj e)
                                    (= existing e) m
                                    :else (assoc m k #{existing e}))))
                normalize-eids (fn [x] (if (set? x) x #{x}))
                tx-av-eids
                (reduce (fn [m {:keys [op datom]}]
                          (if (and (= op :db/add)
                                   (contains? effective-unique-attrs
                                              (:a datom)))
            ...
            (when (seq tx-av-eids)
              (let [releasing-by-av (reduce (fn [m {:keys [op datom]}]
                    ...
                    attrs-with-existing-data (into #{}
                                                   (filter
                                                     attr-has-existing-data?)
                                                   effective-unique-attrs)]
                (doseq [[[a v] eids-or-eid] tx-av-eids]
                  (let [eids (normalize-eids eids-or-eid)]
                    ;; Two distinct entities in the same tx claiming the
                    ;; same unique value.
                    (when (> (count eids) 1)
                      (throw (ex-info "Unique constraint violated"
                                      {:attr a, :val v})))
                    ;; Same-tx retractions release pre-existing unique
                    ;; slots.
        ...
        ;; Track added datoms alongside db state. Schema-defining attrs:
        ;; after any op on these, rebuild caches immediately so subsequent
        ;; ops in the same tx see the correct card-many, ref-attrs, etc.
        tx-schema-touched?
        (or (some (fn [{:keys [datom]}]
                    (contains? schema-defining-attrs (:a datom)))
                  op-datoms)
        ...
        (reduce
          (fn [[db added-acc retracted-acc seen-card-one] {:keys [op datom]}]
            (case op
              :db/add
              (let [card-one? (not (contains? (:card-many db) (:a datom)))
                    ...
                    can-skip-implicit? (and card-one?
                                            new-entity?
                                            (not (contains? seen-card-one
                                                            card-one-key)))
                    implicit
                    (when (and card-one? (not can-skip-implicit?))
                      (vec (seek-ea (:eavt db) (:e datom) (:a datom))))
                    db' (if (seq implicit)
                          (reduce retract-datom-from-indexes db implicit)
                          db)
                    db' (add-datom-to-indexes db' datom)
                    ;; If this op touches schema, rebuild caches now so
                    ;; the next op in this tx uses correct
                    ;; card-many/ref-attrs/indexed-attrs.
                    db' (if (contains? schema-defining-attrs (:a datom))
                          (let [{:keys [schema ref-attrs card-many
                            ...
                            (assoc db'
                                   ...
                                   :indexed-attrs indexed-attrs))
                          db')]
                [db' (conj added-acc datom)
                 (into retracted-acc (or implicit []))
                 (cond-> seen-card-one card-one? (conj card-one-key))])
              :db/retract
              (let [existing (seek-ea (:eavt db) (:e datom) (:a datom))
                    to-retract (vec (filter #(= (:v %) (:v datom))
                                            existing))]
                [(reduce retract-datom-from-indexes db to-retract) added-acc
                 (into retracted-acc to-retract) seen-card-one])
        ...
        ;; Step 12: rebuild schema cache and secondary indexes. Rebuilding
        ;; secondary indexes (AVET/VAET/MEAVT) from EAVT with the final
        ;; schema caches ensures they are consistent even when a same-tx
        ;; :db/retract on a schema-defining attr left a stale entry (e.g.
        ;; keyword v in VAET).
        db-with-log
        (update db' :log conj [t (vec all-tx-datoms) (vec retracted-datoms)])
        db-after (if tx-schema-touched?
                   (let [{:keys [schema ref-attrs card-many unique-attrs
                                 indexed-attrs]}
...
(defn- pad-to-5
  ...
  [clause]
  (let [cnt (count clause)] (into (vec clause) (repeat (- 5 cnt) FREE))))
...
(defn- select-datoms
  ...
  [db e a v]
  (cond (not= e FREE) (seek-ea (:eavt db) e a)
        (and (not= a FREE) (not= v FREE) (avet-attr? db a))
        (seek-av (:avet db) a v)
...
(defn- classify-in-pattern
  ...
  (cond (db-sym? pattern) :db
        ...
        (and (vector? pattern)
             ...
             (= '... (last pattern)))
        :relation
        (and (vector? pattern) (>= (count pattern) 2) (= '... (last pattern)))
        :coll
    ...
    (if-not db
      ...
      (let [[ce ca cv] (pad-to-5 pattern)
        ...
        (cond (and (not= e-val FREE) (not= a-val FREE) (not= v-val FREE)) 1
              (and (not= e-val FREE) (not= a-val FREE)) 2
              (not= e-val FREE) 4
              (and (not= a-val FREE)
                   (not= v-val FREE)
...
(defn- pull-spec
  ...
  (cond (= '* spec) (let [datoms (seek-e (:eavt db) eid)]
                      (reduce (fn [r d]
                                (let [a (:a d)
                                      v (:v d)]
                                  (if (contains? (:card-many db) a)
                              ...
                              datoms))
        (keyword? spec) (if (= :db/id spec)
                          (assoc result :db/id eid)
                          (if-let [v (pull-attr-val db eid spec)]
                            (assoc result spec v)
                            result))
        (map? spec) (reduce (fn [r [ref-attr sub-pattern]]
    ...
    (mapcat (fn [[attr-kw props]]
              (let [eid (tempid!)]
                (cond-> [[:db/add eid :db/ident attr-kw]]
                  (:db/valueType props) (conj [:db/add eid :db/valueType
                                               (:db/valueType props)])
                  (:db/cardinality props) (conj [:db/add eid :db/cardinality
                                                 (:db/cardinality props)])
                  (:db/unique props) (conj [:db/add eid :db/unique
                                            (:db/unique props)])
                  (:db/index props) (conj [:db/add eid :db/index
                                           (:db/index props)]))))
            schema)))
...
(defn create
  ...
  (let [db (empty-db)
    ...
    (if (seq tx-data) (:db-after (run-tx db tx-data)) db)))
...
(defn datoms
  ...
  ([db index] (dao-db/datoms db index))
  ([db index & components] (apply dao-db/datoms db index components)))
...
(extend-type InMemoryDaoDB
  ...
  (write-segment! [this [t added retracted]]
    ;; segment is a [t added retracted] entry — the same format
    ;; read-segments returns. Appends to log, applies additions and
    ;; retractions, advances :next-t, then rebuilds schema caches and
    ;; secondary indexes so the returned db is self-consistent.
    (let [db' (-> this
...
(defn- replay-log-entries
  ...
  (let [entries (vec entries)
        ;; Start with empty indexes AND empty schema caches so
        ;; add-datom-to-indexes never writes stale AVET/VAET entries during
        ;; phase 1.
        base (assoc db
                    :eavt (empty (:eavt db))
                    :aevt (empty (:aevt db))
        ...
        ;; add-datom-to-indexes skips AVET/VAET/MEAVT — correct, we fill
        ;; those later).
    ...
    ;; Phase 3: re-populate AVET/VAET/MEAVT by scanning EAVT with correct
    ;; caches.
    (-> replayed
...
(defn since
  ...
  (let [replayed (->> (:log db)
                      (filter #(> (first %) t))
                      (replay-log-entries db))]
    ;; replay-log-entries rebuilds schema caches from the truncated EAVT,
    ;; which excludes schema defined before the cutoff.  Override with the
    ;; full db's caches, then re-run secondary index rebuild with the
