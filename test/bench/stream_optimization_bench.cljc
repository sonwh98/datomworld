  ...
  (let [state {:store {}, :id-counter 1}
        ...
        cursors (mapv (fn [i]
                        (let [stream-id (+ 1000 i)
                              cursor-id (+ 2000 i)
                              cursor {:stream-ref {:type :stream-ref,
                                                   :id stream-id},
                                      :stream-id stream-id,
                                      :position 0}]
                          [cursor-id cursor]))
          ...
          (loop [remaining-m m
            ...
            (if (zero? remaining-m)
              ...
              (let [stream-idx (mod idx n)
                    ...
                    woken-entries (#'engine/make-woken-run-queue-entries
                                   (:state result)
                                   woken)
                    ;; 3. Update store (advances cursor positions)
                    s' (:state result)
                    store-updates (mapcat (fn [e] (vec (:store-updates e)))
                                          woken-entries)
...
(defn- bench-old-polling
  ...
  (let [state (make-bench-state n capacity)
        ...
        ;; Using transients for vector building to avoid excessive
        ;; allocation
        poll-wait-set
        (fn [s]
          (let [wait-set (:wait-set s)]
            (reduce (fn [acc entry]
                      (let [result (engine/handle-next acc
                        ...
                        (if (:park result)
                          ...
                          (let [updated-store (:store (:state result))
                                ...
                                cursor-update {cursor-id (get updated-store
                                                              cursor-id)}]
                            (-> acc
                                (update :ready-queue-transient
                                        conj!
                                        (assoc entry
                                               :value (:value result)
                    ...
                    (assoc s
                           ...
                           :ready-queue-transient (transient []))
                    wait-set)))
        run-bench
        (fn []
          (loop [remaining-m m
            ...
            (if (zero? remaining-m)
              ...
              (let [stream-idx (mod idx n)
                    ...
                    run-queue (persistent! (:ready-queue-transient s''))
                    s'' (-> s''
                            (dissoc :wait-set-transient
                                    :ready-queue-transient)
                            (assoc :wait-set wait-set
                    ...
                    s''' (if (seq run-queue)
                           (update s''
                                   :wait-set
                                   into
                                   (mapv #(dissoc % :value :store-updates)
                                         run-queue))
                           s'')]
                (recur (dec remaining-m)
                       (inc idx)
                       (assoc s''' :ready-queue []))))))]
    (run-bench)))
