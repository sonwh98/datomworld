...
(deftest tx-pipeline-test
  ...
  (testing "map-m expansion — map :m creates metadata entity"
    (let [db (in-m/empty-db)
          {:keys [db]}
          (in-m/run-tx db [[:db/add 1025 :name "Eve" {:source "test"}]])
          name-datom (first (filter #(= :name (:a %))
                                    (in-m/native-datoms db :eavt)))
  ...
  (testing "tempid resolution — negative eids resolve to positive"
    (let [db (in-m/empty-db)
          {:keys [db tempids]} (in-m/run-tx db
                                            [[:db/add -1 :name "Fred"]
                                             [:db/add -2 :name "Gina"]])]
      (is (= 2 (count tempids)))
  ...
  (testing "cardinality-one: same-tx second :db/add replaces first"
    (let [db (in-m/create {:color {:db/cardinality :db.cardinality/one}})
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add 2048 :color :red]
                                     [:db/add 2048 :color :blue]])
          colors (filter #(= :color (:a %))
  ...
  (testing "cardinality-many: multiple values accumulate"
    (let [db (in-m/create {:tags {:db/cardinality :db.cardinality/many}})
          ...
          tags (map :v
                    (filter #(= :tags (:a %)) (in-m/native-datoms db :eavt)))]
      (is (= #{"a" "b"} (set tags)))))
...
(deftest query-engine-test
  (testing "single variable — find all eids with a given attr"
    (let [db (in-m/empty-db)
      ...
      (is (= #{[1025] [1026]} (dao-db/q '[:find ?e :where [?e :name _]] db)))))
  ...
  (testing "multi-clause join — result is independent of source clause order"
    (let [db (in-m/create {:status {:db/index true},
                           :city {:db/valueType :db.type/ref, :db/index true}})
          {:keys [db]}
          (in-m/run-tx
            db
            [[:db/add 1025 :status :active] [:db/add 1025 :city 2048]
             [:db/add 1026 :status :active] [:db/add 1026 :city 2049]
             [:db/add 1027 :status :inactive] [:db/add 1027 :city 2048]])]
      (is (= #{[1025]}
             (dao-db/q '[:find ?e :in $ ?status ?city :where
                         [?e :status ?status] [?e :city ?city]]
                       db
                       :active
                       2048)))
      (is (= #{[1025]}
             (dao-db/q '[:find ?e :in $ ?status ?city :where [?e :city ?city]
                         [?e :status ?status]]
                       db
                       :active
                       2048)))))
...
(deftest record-test
  (testing "empty-db has bootstrap schema entities (eids 0-14)"
    (let [db (in-m/empty-db)
          ident-datoms (filter #(= :db/ident (:a %))
                               (in-m/native-datoms db :eavt))]
      (is (= 15 (count ident-datoms)))
      (is (= :db/derived (:db/ident (in-m/native-entity db 2))))
      (is (some #(= :db/ident (:v %)) ident-datoms))
      (is (some #(= :db.type/ref (:v %)) ident-datoms))))
  #?(:clj (testing "phase-1 index backend uses persistent-sorted-set"
            (let [db (in-m/empty-db)
                  index-class
  ...
  (testing "entity-attrs accumulates card-many values as vectors"
    (let [db (in-m/create {:tags {:db/cardinality :db.cardinality/many}})
          {:keys [db]}
          (in-m/run-tx db [[:db/add 1025 :tags "a"] [:db/add 1025 :tags "b"]])
          attrs (in-m/native-entity-attrs db 1025)]
  ...
  (testing
    "since inherits pre-cutoff schema so VAET/AVET are populated for post-cutoff data"
    ;; :friend declared as ref at t=1 (before cutoff), data added at t=2
    ;; (after cutoff).
    ;; (since db 1) must still classify :friend as ref and populate VAET.
    (let [db (in-m/empty-db)
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add -1 :db/ident :friend]
  ...
  (testing "as-of shows correct value before cardinality-one overwrite"
    (let [db (in-m/create {:color {:db/cardinality :db.cardinality/one}})
      ...
      (is (= :red
             (:v (first (in-m/native-datoms db-at-2 :eavt [1025 :color])))))
      (is (= :blue
             (:v (first (in-m/native-datoms db-at-3 :eavt [1025 :color])))))))
  ...
  (testing "as-of snapshot has historical schema caches"
    (let [db (in-m/empty-db)
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add -1 :db/ident :tags]
                                     [:db/add -1 :db/cardinality
                                      :db.cardinality/many]])
  ...
  (testing "as-of snapshot populates VAET/AVET using historical ref-attrs"
    ;; Declare :friend as ref at t=1, add data at t=2. As-of at t=2 must
    ;; have :friend in ref-attrs AND datoms in :vaet.
    (let [db (in-m/empty-db)
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add -1 :db/ident :friend]
                                     [:db/add -1 :db/valueType :db.type/ref]]) ; t=1
...
(deftest integration-test
  (testing "load yin.vm/schema, transact (+ 1 2) AST, run SemanticVM"
    (let [;; Create db with yin.vm schema
      ...
      (is (= #{['+]}
             (dao-db/q '[:find ?name :where [_ :yin/name ?name]]
                       (:db loaded-svm))))
      (is (= 3 (vm/value svm))))))
...
(deftest query-in-forms-test
  (testing "collection binding [?name ...] — only listed names returned"
    (let [db (in-m/empty-db)
      ...
      (is (= #{["Alice"] ["Bob"]}
             (dao-db/q '[:find ?name :in $ [?name ...] :where [_ :name ?name]]
                       db
                       ["Alice" "Bob"])))))
  ...
  (testing "tuple binding [?name ?age] — both vars bound"
    (let [db (in-m/empty-db)
          {:keys [db]} (in-m/run-tx
                         db
                         [[:db/add 1025 :name "Alice"] [:db/add 1025 :age 30]
                          [:db/add 1026 :name "Bob"] [:db/add 1026 :age 25]])]
      (is (= #{[1025]}
             (dao-db/q '[:find ?e :in $ [?name ?age] :where [?e :name ?name]
                         [?e :age ?age]]
                       db
                       ["Alice" 30])))))
  (testing "relation binding [[?name ?age] ...] — multi-row expansion"
    (let [db (in-m/empty-db)
          {:keys [db]} (in-m/run-tx
                         ...
                         [[:db/add 1025 :name "Alice"] [:db/add 1025 :age 30]
                          [:db/add 1026 :name "Bob"] [:db/add 1026 :age 25]
                          [:db/add 1027 :name "Carol"] [:db/add 1027 :age 40]])]
      (is (= #{["Alice" 30] ["Bob" 25]}
             (dao-db/q '[:find ?name ?age :in $ [[?name ?age] ...] :where
  ...
  (testing "multi-database cross-join on shared attribute"
    (let [local (in-m/empty-db)
          ...
          {:keys [db]} (in-m/run-tx local
                                    [[:db/add 1025 :email "a@x.com"]
                                     [:db/add 1025 :role "admin"]])
          local db
          {:keys [db]} (in-m/run-tx remote
                                    [[:db/add 2001 :email "a@x.com"]
                                     [:db/add 2001 :score 99]])
          remote db]
      (is (= #{["admin" 99]}
             (dao-db/q '[:find ?role ?score :in $local $remote :where
                         [$local ?e1 :email ?email] [$local ?e1 :role ?role]
  ...
  (testing "scalar + collection cross-join — both constraints apply"
    (let [db (in-m/empty-db)
          ...
          (in-m/run-tx
            db
            [[:db/add 1025 :name "Alice"] [:db/add 1025 :role "admin"]
             [:db/add 1026 :name "Bob"] [:db/add 1026 :role "user"]
             [:db/add 1027 :name "Carol"] [:db/add 1027 :role "admin"]])]
      (is (= #{["Alice"] ["Carol"]}
             (dao-db/q '[:find ?name :in $ ?role [?name ...] :where
                         [?e :name ?name] [?e :role ?role]]
                       db
                       "admin"
                       ["Alice" "Carol" "Bob"]))))))
...
(deftest four-protocol-test
  ...
  (testing
    "write-segment! advances next-eid so later tempids do not reuse imported eids"
    (let [db0 (in-m/empty-db)
          {:keys [db]} (in-m/run-tx db0 [[:db/add 1025 :name "Alice"]])
          entry (first (in-m/read-segments db 1 1))
  ...
  (testing
    "write-segment! advances next-eid when ref-attr is defined in same segment"
    (let [db0 (in-m/empty-db)
          ;; Define :friend as ref and use it with EID 5000 in same segment
          entry [1
                 [(dao-db/->datom 200 :db/ident :friend 1 0)
  ...
  (testing
    "write-segment! rebuilds schema caches and secondary indexes after import"
    ;; Replay schema + data entries through write-segment!; the resulting
    ;; db must have :friend in ref-attrs and the data datom in :vaet.
    (let [src (in-m/empty-db)
          {:keys [db]} (in-m/run-tx src
                                    [[:db/add -1 :db/ident :friend]
                                     [:db/add -1 :db/valueType :db.type/ref]]) ; t=1
          {:keys [db]} (in-m/run-tx db [[:db/add 1025 :friend 1026]])          ; t=2
          entries (in-m/read-segments db 1 2)
  ...
  (testing "IDaoTransactor/transact! applies tx-data"
    (let [db (in-m/empty-db)
      ...
      (is (= "Alice"
             (:v (first (in-m/native-datoms db-after :eavt [1025 :name])))))))
...
(deftest metadata-tempid-resolution-bug
  (testing "Metadata map referencing a tempid from the SAME transaction"
    ;; :actor must be schema-declared as :db.type/ref so the tempid rewrite
    ;; applies. Entity 2048 is used for the action to avoid colliding with
    ;; the schema entity at 1025.
...
(deftest metadata-only-tempid-resolution-bug
  (testing "Metadata map referencing a tempid that only appears in metadata"
    (let [db (in-m/create {:actor {:db/valueType :db.type/ref}})
      ...
      (is
        (= (get tempids -1) actor-val)
        "Metadata-only tempids should be reflected in :tempids and metadata datoms")
      (is (pos? actor-val)
          (str "The tempid -1 should be resolved to a positive ID, but got: "
               actor-val)))))
...
(deftest metadata-keyword-ident-value-resolution-bug
  (testing
    "Metadata ref attrs should resolve keyword idents introduced in the same tx"
    (let [db (in-m/create {:actor {:db/valueType :db.type/ref}})
...
(deftest metadata-m-tempid-resolution-bug
  (testing "A negative metadata entity id in m position resolves as a tx tempid"
    (let [{:keys [db tempids]}
          (in-m/run-tx (in-m/empty-db) [[:db/add 2048 :action "create" -1]])
...
(deftest uniqueness-schema-change-same-tx-bug
  (testing
    "Declaring uniqueness and asserting duplicates in the same tx should fail"
    (let [db (in-m/empty-db)
  ...
  (testing
    "Removing uniqueness and asserting duplicates in the same tx should succeed"
    (let [db (in-m/create {:email {:db/unique :db.unique/value}})
          {:keys [db]} (in-m/run-tx db [[:db/add 1025 :email "a@b.com"]])
          outcome (try {:result (in-m/run-tx db
                                             [[:db/retract :email :db/unique
                                               :db.unique/value]
                                              [:db/add 1026 :email "a@b.com"]])}
                       (catch #?(:clj Exception
                                 :cljs js/Error
...
(deftest uniqueness-value-transfer-same-tx-bug
  (testing
    "Retracting a unique value from one entity and moving it in the same tx should succeed"
    (let [db (in-m/create {:email {:db/unique :db.unique/value}})
          {:keys [db]} (in-m/run-tx db [[:db/add 1025 :email "a@b.com"]])
          outcome (try {:result (in-m/run-tx db
                                             [[:db/retract 1025 :email
                                               "a@b.com"]
                                              [:db/add 1026 :email "a@b.com"]])}
                       (catch #?(:clj Exception
                                 :cljs js/Error
      ...
      (when-let [db-after (:db-after (:result outcome))]
        (is
          (= "a@b.com" (:email (in-m/native-entity-attrs db-after 1026)))
...
(deftest same-tx-ref-schema-for-ordinary-datom-bug
  (testing
    "Same-tx :db.type/ref installation should resolve ordinary ref tempids and populate VAET"
    (let [db (in-m/empty-db)
          tx-data [[:db/add -1 :db/ident :friend]
                   [:db/add -1 :db/valueType :db.type/ref]
                   [:db/add -2 :name "Alice"] [:db/add 2048 :friend -2]]
          {:keys [db tempids]} (in-m/run-tx db tx-data)
          alice-eid (get tempids -2)]
      (is (some? alice-eid) "Alice should get a permanent EID")
      (is
        (= alice-eid (:friend (in-m/native-entity-attrs db 2048)))
...
(deftest stale-secondary-index-bug
  (testing
    "Implicit cardinality/one retractions must be removed from AVET/VAET/MEAVT"
    (let [db (in-m/create {:ssn {:db/index true},
          ...
          ;; 2. Overwrite values (implicit retraction of "111", 1026, and
          ;; m=42)
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add 1025 :ssn "222"]
...
(deftest same-tx-keyword-ident-value-resolution-bug
  (testing
    "Defining a ref attr and target ident in the same tx should resolve keyword values"
    (let [db (in-m/empty-db)
          tx-data [[:db/add -1 :db/ident :friend]
                   [:db/add -1 :db/valueType :db.type/ref]
                   [:db/add -2 :db/ident :bob] [:db/add 1025 :friend :bob]]
          {:keys [db tempids]} (in-m/run-tx db tx-data)
          bob-eid (get tempids -2)
          friend-datom (first (in-m/native-datoms db :eavt [1025 :friend]))]
      (is (pos? bob-eid) "Bob should have a positive permanent EID")
      (is
        (= bob-eid (:v friend-datom))
...
(deftest ref-attr-av-lookup-uses-avet
  (testing "Ref attr [a v] query and lookup can use AVET without AEVT"
    (let [db (in-m/create {:friend {:db/valueType :db.type/ref}})
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add 1025 :friend 1026]
                                     [:db/add 1027 :friend 1028]])
          ;; This pins the index-selection contract without
      ...
      (is (= #{1025} (in-m/native-find-eids-by-av db-without-aevt :friend 1026))
...
(deftest uniqueness-check-uses-avet-when-available
  (testing "Unique conflict checks use AVET for indexed and ref attrs"
    (doseq [[label schema tx-data conflict]
            [["indexed unique attr"
              {:email {:db/unique :db.unique/value, :db/index true}}
              [[:db/add 1025 :email "a@b.com"]]
              [[:db/add 1026 :email "a@b.com"]]]
             ["ref unique attr"
              {:owner {:db/valueType :db.type/ref,
                       :db/unique :db.unique/value}}
              [[:db/add 1025 :owner 2048]] [[:db/add 1026 :owner 2048]]]]]
...
(deftest same-tx-ref-schema-retraction-bug
  (testing
    "Retracting :db.type/ref in the same tx should stop ref resolution immediately"
    (let [db (in-m/create {:friend {:db/valueType :db.type/ref}})
          {:keys [db]} (in-m/run-tx db [[:db/add 1026 :db/ident :bob]])
          {:keys [db]} (in-m/run-tx db
                                    [[:db/retract :friend :db/valueType
                                      :db.type/ref]
      ...
      (is
        (empty? (in-m/native-datoms db :vaet))
...
(deftest type-rank-nil-comparison-bug
  (testing "compare-vals with nil"
    ...
    (is (pos? (dao-db/compare-vals "a" nil))
        "string should be greater than nil")))
...
(deftest as-of-schema-flip-bug
  (testing "Historical state after attribute changes from :many to :one"
    ...
    ;; as-of t=2: Alice should still have ["a" "b"] as a vector (historical
    ;; :many)
    (let [db (in-m/create {:tags {:db/cardinality :db.cardinality/many}})
          {:keys [db]}
          (in-m/run-tx db [[:db/add 1026 :tags "a"] [:db/add 1026 :tags "b"]]) ; t=2
          ;; Explicitly change schema at t=3 using ident-based entity
          ;; reference
          {:keys [db]} (in-m/run-tx db
                                    [[:db/add :tags :db/cardinality
                                      :db.cardinality/one]]) ; t=3
          snap (in-m/as-of db 2)
          attrs (in-m/native-entity-attrs snap 1026)]
      (is (vector? (:tags attrs))
          "Historical attributes should respect historical cardinality")
      (is (= #{"a" "b"} (set (:tags attrs)))
          "Should return both values from t=2"))))
...
(deftest explicit-positive-eid-run-tx-allocation-bug
  (testing
    "run-tx should advance next-eid past explicit positive user entity ids"
    (let [db0 (in-m/empty-db)
          {:keys [db]} (in-m/run-tx db0 [[:db/add 1025 :name "Alice"]])]
      (is
        (= 1026 (:next-eid db))
