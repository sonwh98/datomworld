(ns dao.await.v2-test
  "Tests for dao.await.v2 — the v1 await suite, ported to yin.vm.v2 and
   dao.stream.v2.
  ...
  (:require [clojure.test :refer [deftest is testing]]
            [dao.await.v2 :as await]
            [dao.stream.v2 :as ds]
            [dao.stream.v2.ringbuffer :as ringbuffer])
  #?(:cljs (:require-macros [dao.await.v2])))
...
(deftest go-returns-process-descriptor-test
  (testing "go yields a :dao.await.v2/process map with ast + datoms"
    (let [proc (await/go 42)]
      (is (= :dao.await.v2/process (:type proc)))
      (is (some? (:ast proc)))
