...
(deftest canonical-path-follows-the-wire-spec
  ...
  ;; the client canonicalizer (`yin.repl.v2.connect`) agrees.
  ...
  (let [traffic (buffer)
    ...
    (let [attach (attacher (descriptor))
      ...
      (let [polled (rpc/poll! client 4)]
        (is (= :dao.stream.v2.rpc/idle (:dao.stream.v2.rpc/outcome polled)))
        (is (nil? (:terminal (:dao.stream.v2.rpc/state polled))))
        (is (empty? (:completed (:dao.stream.v2.rpc/state polled))))
        (let [requested (rpc/request! (:dao.stream.v2.rpc/state polled)
                                      :op/echo ["hi"])]
          ...
          (let [answered (rpc/poll! (:dao.stream.v2.rpc/state requested) 4)
                [completions next-state] (rpc/take-completed
                                           (:dao.stream.v2.rpc/state answered))]
            (is (= 1 (count completions)))
            (is (= 0 (:dao.stream.v2.rpc/id (first completions))))
            (is (= [:echo "hi"]
                   (apply/response-ok (:dao.stream.v2.rpc/response (first completions)))))
  ...
  (doseq [[attachment {:keys [requests socket] :as entry}] @(:services fixture)]
    (let [result (apply/serve-once! echo-handlers requests socket (:service entry))]
      (swap! (:services fixture) assoc-in [attachment :service]
             (:dao.stream.v2.apply/state result)))))
  ...
  (let [st @(:rpc-atom client)]
    (when (:unsent st)
      ...
      (reset! (:rpc-atom client)
              (:dao.stream.v2.rpc/state (rpc/request! st ::retry nil))))
    (let [polled (rpc/poll! @(:rpc-atom client) 16)]
      (reset! (:rpc-atom client) (:dao.stream.v2.rpc/state polled))
                          ...
                          (let [client (make-client (assoc (:descriptor fixture) :ws/port port))
                            ...
                            (let [initial (rpc/request! @(:rpc-atom client) :op/echo ["hello"])]
                              (is (= :dao.stream.v2.rpc/pending-request
                                     (:dao.stream.v2.rpc/outcome initial)))
                              (reset! (:rpc-atom client) (:dao.stream.v2.rpc/state initial)))
                            (after 4000 (fn [] (seq @(:completed client)))
                                   "no completion arrived" fixture server [client] finish
                                   (fn [_]
                                     (let [result (first @(:completed client))]
                                       (is (= 0 (:dao.stream.v2.rpc/id result)))
                                       (is (= [:echo "hello"]
                                              (apply/response-ok (:dao.stream.v2.rpc/response result))))
                                   ...
                                   (fn [_]
                                     ...
                                     (after 4000 (fn []
                                            ...
                                            (fn [terminal]
                                              ...
                                              (is (not= :dao.stream.v2.apply/detached terminal))
                                              (teardown! fixture [client] server ticker)
                                              (finish)))))))))))
                            ...
                            (after 4000 (fn []
                                   ...
                                   (fn [_]
                                     ...
                                     (is (= :dao.stream/closed
                                            (:dao.stream.v2.rpc/reason (first @(:completed client)))))
                                                         ...
                                                         (fn [_]
                                                           (let [last-done (last @(:completed client))
                                                                 openings (filter #(= :ws/opened (:ws/event %))
                                                                                  (drain (:traffic client)))]
                                                             (is (= 1 (:dao.stream.v2.rpc/id last-done)))
                                                             (is (= [:echo "again"]
                                                                    (apply/response-ok
                                                                      (:dao.stream.v2.rpc/response last-done))))
