  ...
  (testing "State has all required fields"
    (let [state (make-state {})]
      ...
      (is (contains? state :environment)
          "State should have :environment field")
      (is (contains? state :store)
          "State should have :store field")
      (is (contains? state :continuation)
          "State should have :continuation field")
      (is (contains? state :value)
          "State should have :value field"))))
  ...
  (testing "Initial state has correct default values"
    (let [state (make-state {})]
      ...
      (is (empty? (:environment state))
          "Initial environment should be empty")
      (is (empty? (:store state))
          "Initial store should be empty")
      (is (nil? (:continuation state))
          "Initial continuation should be nil")
      (is (nil? (:value state))
...
(deftest test-state-immutability
  (testing "State is immutable - eval creates new state"
    (let [ast {:type :literal :value 42}
      ...
      (is (nil? (:value state-0))
          "Original state value should be unchanged")
      (is (nil? (:control state-1))
          "New state control should be nil")
      (is (= 42 (:value state-1))
          "New state value should be 42"))))
  ...
  (testing "State transitions for literal evaluation"
    (let [ast {:type :literal :value 42}
      ...
      (testing "Initial state"
        ...
        (is (nil? (:value state-0))
            "Initial state should have no value"))

      (testing "After one step"
        (let [state-1 (vm/eval state-0 nil)]
          (is (nil? (:control state-1))
              "After eval, control should be nil")
          (is (= 42 (:value state-1))
              "After eval, value should be 42"))))))
    ...
    (let [add-fn (get vm/primitives '+)
      ...
      (testing "Final state has result"
        (let [final-state (last all-states)]
          (is (nil? (:control final-state))
              "Final state control should be nil")
...
(deftest test-state-step-count
  (testing "Count evaluation steps via state transitions"
    (let [add-fn (get vm/primitives '+)
      ...
      (is (= 17 step-count)
          "Should take exactly 17 steps to evaluate"))))
