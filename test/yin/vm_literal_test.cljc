  ...
  (testing "Integer literals evaluate to themselves"
    (let [ast {:type :literal :value 42}
      ...
      (is (nil? (:control result))
          "Control should be nil after evaluation")
...
(deftest test-boolean-literals
  (testing "Boolean literals evaluate to themselves"
    (let [true-ast {:type :literal :value true}
          ...
          true-result (vm/run (vm/make-state {}) true-ast)
          false-result (vm/run (vm/make-state {}) false-ast)]
      (is (true? (:value true-result))
          "true should evaluate to true")
      (is (false? (:value false-result))
  ...
  (testing "Floating point literals evaluate to themselves"
    (let [ast {:type :literal :value 3.14159}
      ...
      (is (= 3.14159 (:value result))
          "Float should evaluate to itself"))))
