...
(defn- fact-values
  [datoms attr]
  (mapv #(nth % 2) (filter #(= attr (nth % 1)) datoms)))
...
(defn- sample-ast
  ...
  {:type :application,
   ...
   :operands [{:type :literal, :value 20} {:type :literal, :value 22}]})
  ...
  (let [constructors {:ast-walker ast-walker/create-vm,
    ...
    (doseq [[model create-vm] constructors]
      (testing (str "Telemetry covers " model)
        (let [telemetry-stream (make-stream)
              vm-result (vm/eval (create-vm {:telemetry {:stream
                                                         telemetry-stream,
                                                         :vm-id model}})
                                 (sample-ast))
              datoms (stream-values telemetry-stream)]
...
(deftest bridge-step-emits-bridge-phase-and-op-test
  (let [telemetry-stream (make-stream)
        vm-result (vm/eval (ast-walker/create-vm
