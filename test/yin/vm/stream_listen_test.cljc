  ...
  (testing
    ...
    (doseq [[vm-type create-vm] (datom-vms)]
      (let [in-stream (ds/open! {:type :ringbuffer, :capacity nil})
        ...
        (ds/append! in-stream read-x-program)
        (let [result (vm/run vm0)]
          (is (= 42 (vm/value result))
              (str vm-type
                   " should preserve store across sequential ingress programs"))
          (is (= {:position 2} (:in-cursor result))
              (str vm-type
                   " should advance ingress cursor after both batches")))))))
      ...
      (let [in-stream (ds/open! {:type :ringbuffer, :capacity nil})
        ...
        (let [result (vm/run vm0)]
          (is (= 41 (vm/value result))
              (str vm-type
                   " should return the value of the final ingested program"))
          (is (vm/halted? result)
              (str vm-type " should halt once ingress reaches :end"))
          (is (= {:position 1} (:in-cursor result))
              (str vm-type
                   " should advance ingress cursor before stream end")))))))
