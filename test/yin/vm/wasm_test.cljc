...
#?(:cljs
   (do
     ...
     (deftest eval-arithmetic-test
       (testing "(+ 3 4) -> 7.0"
         (is (= 7.0
                (vm/value (vm/eval (wasm/create-vm)
                                   (app '+ (lit 3) (lit 4)))))))
       (testing "(- 10 3) -> 7.0"
         (is (= 7.0
                (vm/value (vm/eval (wasm/create-vm)
       ...
       (testing "(/ 10 4) -> 2.5"
         (is (= 2.5
                (vm/value (vm/eval (wasm/create-vm)
                                   (app '/ (lit 10) (lit 4)))))))
       (testing "(* (+ 1 2) (- 5 3)) -> 6.0"
         (let [ast (app '* (app '+ (lit 1) (lit 2)) (app '- (lit 5) (lit 3)))]
           (is (= 6.0 (vm/value (vm/eval (wasm/create-vm) ast)))))))
     (deftest eval-comparison-test
       (testing "(= 1 1) -> true"
         (is (true? (vm/value (vm/eval (wasm/create-vm)
                                       (app '= (lit 1) (lit 1)))))))
       (testing "(= 1 2) -> false"
         (is (false? (vm/value (vm/eval (wasm/create-vm)
                                        (app '= (lit 1) (lit 2)))))))
       (testing "(< 1 2) -> true"
         (is (true? (vm/value (vm/eval (wasm/create-vm)
                                       (app '< (lit 1) (lit 2)))))))
     ...
     (deftest eval-if-test
       ...
       (testing
         "mixed branches promotion: (if false 42.0 true) -> 1.0 (promoted boolean)"
         (let [ast {:type :if,
                    :test (lit false),
                    :consequent (lit 42.0),
                    :alternate (lit true)}]
     ...
     (deftest eval-unresolved-variable-test
       (testing
         "eval throws on a variable reference outside the enclosing fn's params"
         (let [ast (yang/compile-program
                     '[(def leaky (fn [n] (+ n undefined-var))) (leaky 1)])]
           (is (thrown-with-msg? js/Error
                                 #"unresolved variable"
                 (vm/eval (wasm/create-vm) ast))))))))


(deftest wasm-boundary-and-error-paths-test
  (testing "ast-datoms->asm throws on unknown operator"
    (let [ast (app 'unknown-op (lit 1) (lit 2))]
      (is (thrown-with-msg? #?(:cljs js/Error
                               :cljd Object
                               :default Exception)
                            #"unsupported primitive operator"
            (wasm/ast-datoms->asm (vm/ast->datoms ast))))))
  (testing "assemble throws on unrecognized WASM instruction IR"
    (let [invalid-ir {:type :f64, :instrs [[:invalid.op 42]]}]
      (is (thrown? #?(:cljs js/Error
                      :cljd Object
                      :default Exception)
            (wasm/assemble invalid-ir)))))
  (testing "ast-datoms->asm throws on an AST node type it doesn't compile"
    (let [ast {:type :lambda, :params ['n], :body (lit 1)}]
      (is (thrown-with-msg? #?(:cljs js/Error
                               :cljd Object
                               :default Exception)
