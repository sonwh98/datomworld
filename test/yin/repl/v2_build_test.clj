(ns yin.repl.v2-build-test
  "The v2 REPL's host entry points are named by build configuration, so the
   v2 entries are checked here — including that the deleted v1 entries they
   replaced are absent."
...
(deftest deps-edn-carries-the-four-additive-v2-aliases
  (let [aliases (:aliases (read-config "deps.edn"))]
    ...
    (testing "the v1 aliases are deleted"
      (is (not (contains? aliases :clj-yin-repl)))
      (is (not (contains? aliases :cljs-yin-repl)))
      (is (not (contains? aliases :telemetry-server)))
      (is (not (contains? aliases :cljd-yin-repl)))
      (is (not (contains? aliases :cljd-yin-repl-build))))))
...
(deftest shadow-cljs-edn-carries-the-additive-node-build
  (let [builds (:builds (read-config "shadow-cljs.edn"))]
    ...
    (testing "the v1 node builds are deleted"
      (is (not (contains? builds :yin-repl)))
...
(deftest the-cljd-entry-point-exists-and-names-the-v2-namespace
  (let [entry (slurp (io/file "bin/yin_repl_v2_main.dart"))]
    (is (not (.exists (io/file "bin/yin_repl_main.dart")))
        "the v1 entry is deleted")
