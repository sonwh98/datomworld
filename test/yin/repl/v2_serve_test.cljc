...
(defn- texts
  [endpoint]
  (mapv :yin.repl.v2.serve/text (first (serve/take-outbox endpoint))))
...
(deftest two-clients-each-get-their-own-answers
  (let [{:keys [endpoint]} (endpoint!)
    ...
    (let [endpoint (serve/step endpoint 6)
      ...
      (is (= [{:dao.stream.v2.apply/id 7 :dao.stream.v2.apply/ok "2"}] (values a)))
      (is (= [{:dao.stream.v2.apply/id 9 :dao.stream.v2.apply/ok "4"}] (values b))))))
...
(deftest an-unknown-operation-is-a-portable-error-not-a-thrown-handler
  (let [{:keys [endpoint]} (endpoint!)
    ...
    (let [_ (serve/step endpoint 4)
      ...
      (is (= :yin.repl.v2.serve/unknown-operation
             (:dao.stream.v2.apply/code (apply/response-error response)))
...
(deftest incomplete-input-is-answered-and-never-crosses-an-attachment-boundary
  (let [{:keys [endpoint]} (endpoint!)
    ...
    (let [endpoint (serve/step endpoint 6)
      ...
      (is (= :yin.repl.v2.serve/incomplete-input
             (:dao.stream.v2.apply/code (apply/response-error response)))
