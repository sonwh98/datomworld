...
(deftest test-tokenize
  (testing "Tokenizing Python source"
    ...
    (is (= [[:keyword "lambda"] [:identifier "x"] [:colon ":"]
            [:identifier "x"] [:operator "*"] [:number "2"]]
  ...
  (testing "Python and Clojure compile to equivalent ASTs"
    ...
    (testing "Function calls"
      (let [py-ast (py/compile "f(1, 2)")
        ...
        (is (= (count (:operands py-ast))
               (count (:operands clj-ast))
               2))))))
...
(deftest test-error-handling
  (testing "Error handling for invalid Python"
    ...
    (testing "Invalid syntax"
      (is (thrown? Exception
                   (py/compile "def"))))  ;; Incomplete def statement
