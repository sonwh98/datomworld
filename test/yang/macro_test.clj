...
(defn compile-program-and-run-macros
  ...
  (let [ast (yang/compile-program (concat macro/stdlib-forms forms))
    ...
    (-> (queue-vm (semantic/create-vm {:macro-registry
...
(deftest test-defn-macro
  (testing "defn defines a named function"
    (is (= 6
           (compile-program-and-run-macros '((defn double
                                               [x]
                                               (* x 2))
                                             (double 3))))))
  (testing "defn with multiple params"
    (is (= 7
           (compile-program-and-run-macros '((defn add
                                               [x y]
                                               (+ x y))
                                             (add 3 4))))))
  (testing "defn with multiple body forms (do desugaring)"
    (is (= 6
           (compile-program-and-run-macros '((defn f
                                               [x]
                                               (def y 1) (+ x y))
                                             (f 5))))))
  ...
  (testing "defn and def can coexist"
    (is (= 9
           (compile-program-and-run-macros
             '((def base 3) (defn square
                              [x]
                              (* x x)) (square base))))))
  (testing "defn param name shadows outer macro name"
    ;; (defn 3) inside the body should call the param, not trigger
    ;; macro-expand
    (is (= 3
           (compile-program-and-run-macros
             '((defn call
                 [defn]
                 (defn 3)) (def id (fn [x] x)) (call id))))))
  (testing "defn tail-recursive countdown (deep enough to overflow without TCO)"
    ;; 100,000 iterations: would stack-overflow without tail-call
    ;; optimisation
    (is (= 0
           (compile-program-and-run-macros
             '((defn countdown
               ...
               (countdown 100000))))))
  (testing
    "defn tail-recursive countdown preserves tail markers through a multi-form body"
    ...
    (let [forms (concat
      ...
      (is (every? (comp true? :tail) countdown-calls)
          (str "expected all countdown call sites to be tail-marked, got "
               countdown-calls))))
  ...
  (testing "user (defmacro defn ...) overrides the native defn special case"
    ...
    ;; (defn ...)
    ;; must expand via the user macro, not the native compiler branch. The
    ;; macro returns 42 directly; (defn x [y] y) evaluates to 42 (no x
    ;; binding).
    (is (= 42
           (compile-program-and-run-macros
             '((defmacro defn
...
(deftest test-macro-builder-primitives
  (testing
    "yin/get-attr can inspect nodes built earlier in the same macro expansion"
    (is
      (=
        :lambda
        (compile-program-and-run-macros
          '((defmacro inspect-lambda
              [ps]
              (let
                [b (yin/sequence-body []) l (yin/make-lambda ps b)]
