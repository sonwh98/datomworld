...
Folding several streams appends a namespace slot: `[e a v t m ns]`. The short form is a literal prefix of
the long one, so a stream stores five slots and only a cross-stream query materializes the sixth.
