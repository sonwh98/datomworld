...
(defn short-name
  "Abbreviate namespace: datomworld.core -> d.core, yin.vm.stack -> y.v.stack"
  [ns-sym]
  (let [parts (str/split (str ns-sym) #"\.")
        n (count parts)]
    (if (<= n 1)
      (str ns-sym)
      (str (str/join "." (map #(subs % 0 1) (butlast parts)))
           "."
           (last parts)))))
...
(defn -main
  ...
  (let [opts (parse-args args)
        ...
        out-file "dep-graph.dot"]
    (spit out-file dot-str)
